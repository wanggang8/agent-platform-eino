...
| --- | --- | --- | --- | --- | --- |
| Batch D 六个工具进入 mock/参数化 provider catalog | `docs/fobrain-live-read-batch-plan.md`、`tool-matrix-24.json` | `internal/einoapp/providers/fobrain/batch_d_test.go` | `go test ./internal/einoapp/providers/fobrain -count=1` | Pass | 只有 client 实现 `ParameterizedQueryClient` 时才注册 Batch D；本记录创建时真实 HTTP client 尚未实现该接口，后续已由 `phase-8-batch-d-live-normalization-2026-07-02.md` 接上 live mapper。 |
...
- 本记录只覆盖 mock/catalog 前置切片；真实 HTTP client mapper 后续已由 `phase-8-batch-d-live-normalization-2026-07-02.md` 补充。
- Batch D 尚未补齐专用 live report、真实分页批量验收、实体消歧恢复数据和 Workbench 视觉证据。
