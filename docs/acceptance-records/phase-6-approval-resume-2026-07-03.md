...
本记录覆盖 Phase 6.2 approval interrupt/resume 后端切片：approval-required capability 创建 waiting pending、审批前不执行 mutation、安全 `resume_ref:` / `checkpoint_ref:`、内部 continuation checkpoint、approve 后恢复并执行一次 capability、reject 后不可 approve、cancel 后取消整个 run 且不可 approve、duplicate resume 幂等、进程重启后继续执行，以及 Product Facts audit/replay 事实入库。
...
| cancel 后不可 approve | `TestApprovalInterruptResumeCancelIsIdempotentAndCannotApprove` 覆盖 cancelled、duplicate cancel 和后续 approve 拒绝 | 通过 |
| cancel 不依赖 checkpoint | `TestApprovalInterruptResumeCancelDoesNotRequireCheckpoint` 覆盖 checkpoint 丢失时仍可取消整个 run | 通过 |
...
| restart cancel | `TestApprovalInterruptResumeCancelAfterRestartClosesRun` 使用 SQLite Product Facts 重启后取消 run 且不执行 mutation | 通过 |
...
go test ./internal/einoapp/execution -run 'ApprovalInterruptResumeCancel' -count=1
...
- Phase 6.2 只完成后端 approval resume；Pending UI/SSE patch、真实 Eino graph interrupt resume 和视觉验收仍需后续任务完成。
...
- reject 后 run 使用 `failed` + `safe_error=approval_rejected`，cancel 后 run 使用 `cancelled` + `safe_error=approval_cancelled`；如产品需要单独的 user-declined 视觉状态，应在投影层新增安全映射，不应改变 provider 或 checkpoint 事实。
