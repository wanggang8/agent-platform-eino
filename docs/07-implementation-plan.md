...
- 当前 Batch B 切片已覆盖 `my_assets`、`my_department_assets`、`my_vulnerabilities`、`my_department_vulnerabilities`、`my_business_systems`、`my_important_business_systems` 六个“我的范围”工具的 catalog、输入 mapper、mock client、StructuredResult 安全摘要、HTTP live mapper focused tests 和 `fobrain-batch-b` live smoke/report 基础设施。HTTP mapper 复用当前用户上下文派生 owner/department；业务系统使用 `person_base.name` 作为 owner scope 条件，用户 keyword 只作为 `business_name` 收窄条件，重要性按 `assets_attribute.important_types` 过滤。2026-07-06 真实配置下，Batch B live smoke 因当前用户上下文缺少可解析部门且“我的范围”查询为空而 blocked。该结论不覆盖真实 passed live report、视觉、Action API 同源 smoke 或 24/24 完成。
- 当前 Batch C 切片已覆盖 `business_list`、`external_high_risk_assets`、`vulnerability_status_summary`、`pending_tickets`、`ip_stats`、`vul_stats` 六个“直接列表与统计”工具的 catalog、安全筛选 mapper、mock client、StructuredResult 安全摘要、HTTP live mapper focused tests，以及 `fobrain-batch-c` live smoke/report 基础设施。2026-07-06 真实配置下，除 `pending_tickets` 因当前环境无待处理工单功能数据而按用户确认标记为 `skipped` 外，其余 5 个工具 live smoke 已通过；该结论不覆盖视觉、Action API 同源 smoke 或 24/24 完成。
...
go test ./scripts/fobrain_sample_discovery ./scripts/fobrain_batch_b_smoke ./scripts/fobrain_batch_c_smoke ./scripts/fobrain_batch_e_smoke -count=1
npm run eino-workbench:schema-test
