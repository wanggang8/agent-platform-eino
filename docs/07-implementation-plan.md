...
- 当前 Batch C mock/catalog 切片已覆盖 `business_list`、`external_high_risk_assets`、`vulnerability_status_summary`、`pending_tickets`、`ip_stats`、`vul_stats` 六个“直接列表与统计”工具的 catalog、安全筛选 mapper、mock client 和 StructuredResult 安全摘要。该结论只覆盖 mock/catalog，不覆盖 HTTP live mapper、视觉、Action API 同源 smoke 或 24/24 完成。
