...
| `business_risk_summary` | `/api/v1/threat_center/count` | POST | body 数组：`count_name=business_risk`、`aggregation_field=business.name.keyword`、`data_range=4`、`search_condition` 使用 `business_name` 过滤 | 旧 adapter `BusinessRiskSummary`；Fobrain `threat_center.Count`、`threat.Count`；真实部署探针 | 不走自然语言总结；只把统计 bucket 归一为安全 metrics。 |
...
当前代码进展：步骤 1-5 已完成 provider catalog、输入 mapper、mock StructuredResult、HTTP mapper、sample discovery 本地样本扩展、Batch E live smoke 脚本、`batch_e_live_report` schema、真实环境脱敏 live pass 报告，以及 Workbench 视觉、fresh replay、audit evidence。HTTP mapper 已覆盖资产详情 `network_type` 中央归一、漏洞详情非分页响应、业务风险 count POST body、威胁关联 `keyword/vul_name` 双参数映射和 `/api` 到 `/api/v1` fallback。业务风险过滤字段按真实部署使用 `business_name`，聚合字段仍为 `business.name.keyword`。Batch E 当前切片可声明完成；Fobrain 24 只读恢复仍需 Batch B-D 视觉、Action API 真实服务同源 smoke、实体消歧和写域审批等后续门禁。
...
- `business_risk_summary` 若真实部署不支持 `/threat_center/count` 的 `business.name.keyword` 聚合或 `business_name` 过滤，必须记录 `connector_execution_failed`，不得由模型编造摘要。
