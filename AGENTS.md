...
- 代码必须保持清晰分层：
  ...
  - `llm`：只处理模型 provider interface、配置、网络策略、mock provider 和错误脱敏。
...
- 不创建万能 `utils` 包；公共能力必须按职责归属到 `bootstrap`、`httpapi`、`llm`、`capabilities`、`facts`、`product` 或 `observability`。
...
- 新增或修改代码必须使用中文注释说明关键设计意图；公共函数、导出类型、接口、复杂状态机、事务边界和安全边界必须有简短中文说明。
- 测试中的复杂业务场景、回归边界和安全断言必须有中文注释说明目的；不要用无意义注释复述代码。
